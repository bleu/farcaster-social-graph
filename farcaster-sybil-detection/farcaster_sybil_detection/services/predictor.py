...
class Predictor:
    ...
    def _generate_prediction(self, features: pl.DataFrame) -> Dict[str, Any]:
        try:
            # Get the expected feature columns from the model
            ...
            for col in expected_features:
                if col not in features.columns:
                    features = features.with_columns(pl.lit(0).alias(col))
            ...
            self.logger.info(f"Predicting for features: {feature_cols}")
            self.logger.info(f"Model features: {self.model.feature_names}")
