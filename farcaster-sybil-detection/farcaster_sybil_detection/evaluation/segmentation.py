...
class UserSegmentation:
    ...
    def segment_users(self, df: pl.DataFrame) -> Dict[str, pl.DataFrame]:
        ...
        for name, segment in self.segments.items():
            try:
                ...
                if missing_cols:
                    print(
                        f"Warning: Skipping {name} segment due to missing columns: {missing_cols}"
                    )
                    continue
                ...
                if size > 0:
                    ...
                    metrics_to_print = [
                        col for col in available_metrics if col in df.columns
                    ]
    ...
    def prepare_segment_features(
        self, segment_df: pl.DataFrame, segment_name: str
    ) -> Tuple[np.ndarray, List[str]]:
        """Prepare features for a specific segment"""
        segment = self.segments[segment_name]
        valid_features = [
            col for col in segment.feature_names if col in segment_df.columns
        ]
